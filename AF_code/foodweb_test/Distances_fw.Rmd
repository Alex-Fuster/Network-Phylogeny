...
mat <-  list_networks_sppnames_letters[[20]][names(which(presence_matrix[20,] == 1)), names(which(presence_matrix[20,] == 1))]


compute_nmi <- function(matrix) {
...
colnames(matrix_nmi) <- colnames(mat)
rownames(matrix_nmi) <- rownames(mat)
...
#  Get result
matrix_nmi <- matrix(0, ncol = ncol(mat), nrow = nrow(mat))
...
if (length(which(unlist(lapply(list_dist_chisq, is.null)) == TRUE)) == 0) {
...
if (length(list_dist_chisq) == length(list_dist.phylo)) {
  
  print("all good - same length list of interaction distances and list of phylo distances")
} else {
