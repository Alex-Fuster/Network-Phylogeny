...
## OVERALL DISTANCES (MEAN between AS PRED AND PREYS) (rows)
...
for (i in 1:length(list_interact_distances_pred)) {
 ...
 pair_mat <- list(list_interact_distances_pred[[i]], list_interact_distances_prey[[i]])
...
list_interact_distances_mean <- lapply(list_interact_distances_mean, FUN = diag_to0)
...
if(length(list_dist.phylo) == length(list_interact_distances_pred) &
   ...
   length(list_dist.phylo) == length(list_interact_distances_mean)) {
...
list_interact_distances_mean <- list_interact_distances_mean[-which(vec_discard == "discard")]
...
if (length(which(check_colnames_lists(list_dist.phylo_pres, list_interact_distances_mean) == FALSE)) == 0) {
  
  print("all good - names in phylo dist matrices and interaction matrices mean match")
...
list_dist.phylo_pres1 <- list_dist.phylo_pres[-1]

list_interact_distances_pred_corrected1 <- list_interact_distances_pred_corrected[-1]

list_interact_distances_prey_corrected1 <- list_interact_distances_prey_corrected[-1]

list_interact_distances_mean_corrected1 <- list_interact_distances_mean_corrected[-1]



for (i in 1:length(list_dist.phylo_pres[-1])) {
...
presence_matrix <- list_simulation1$presence_matrix[5:length(list_simulation1$list_anc_dist),] # select the timesteps we are analysing (from 8 in advance)
