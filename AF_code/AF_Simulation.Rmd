...
source("../AF_code/AF_functions_M-C_bifurcation.R")
	...
	for(int in 0:1){
		...
		while(successful_sim < nsim+1){
  		...
  		if(success >= 20){
    		...
    		res_sim <- list(simulation_number = paste0("simulation", successful_sim), seed = seed, parameters = pars, presence_matrix = simulation$pres, traits_df = simulation$traits, parentage_matrix = simulation$anc, extinxction_matrix = simulation$extinct, network_list = simulation$L_list) # Record results from the simulation
