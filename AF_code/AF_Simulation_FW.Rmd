...
source("../AF_code/AF_functions_M-C_bifurcation _FW.R")
...
#for(b in 1:10){
	...
	b <- 1
		...
		while(successful_sim < nsim+1){
  		...
  		if(success >= 20){
    		...
    		res_sim <- list(simulation_number = paste0("simulation", successful_sim),
    		                ...
    		                mat_ext_prob = simulation$mat_ext_prob, 
    		                mat_Pext_npred = simulation$mat_Pext_npred,
    		                mat_Pext_sharedpred = simulation$mat_Pext_sharedpred) # Record results from the simulation
		...
		# Save simulations results data
			...
			path <- paste0("../AF_Data/Positive_interactions/foodwebTest", nsim, ".Rdata")
...
list_res_pos_0B and list_res_neg_0B are results with Bspe and Bext = 0 (so no minimum number of interactions forced - they can loose the interctions)
