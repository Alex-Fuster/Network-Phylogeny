...
#### compute shared predators


compute_shared_predators <- function(mat) {
  
  
  npred_total = length(which(colSums(mat) > 0)) #total number of predators
  vec_npred_spp = rowSums(mat) # number of predators for each species
  
  
  # only for those spp with predators (vec_npred_spp), obtain a measure of shared predators dividing its number by the total amount of predators.
  
  for (i in 1:length(vec_npred_spp)) {
    
    if(i > 0){
      
      vec_npred_spp[i] <- vec_npred_spp[i]/npred_total
...
sim_model_bif = function(seed, pars, nsteps) {
	...
	mat_ext_prob <- matrix(0, nrow = nsteps, ncol=Smax)
	mat_Pext_npred <- matrix(0, nrow = nsteps, ncol=Smax)
	mat_Pext_sharedpred <- matrix(0, nrow = nsteps, ncol=Smax)
	...
	for(step in 2:nsteps) {
		...
		#### Increasing with the number of predators
		
		Pext_npred <- numeric(length(1:Smax))
		
		for (i in 1:nrow(L)) {
		  Pext_npred[i] <- 1 + -1 * exp(-0.45 * sum(L[i,]))
		}
		
		
		
		#Pext_npred <- Pext_npred[grep(1,pres[step,])]
		
		#### Decreasing with the number of shared predators
		
		shared_pred <- compute_shared_predators(mat = L)
		
		Pext_sharedpred <- numeric(length(1:Smax))
		
		for (i in 1:length(shared_pred)) {
		  
		  Pext_sharedpred[i] <- 0.075 + 5.19 * exp(-10 * shared_pred[i])
		  
		}
		
		#Pext_sharedpred <- Pext_sharedpred[grep(1,pres[step,])]
		

		
		#### total probability
		
		ext_prob <- Pext_sharedpred # Pext_npred + Pext_sharedpred
		
		mat_Pext_sharedpred[step,] <- Pext_sharedpred
		mat_Pext_npred[step,] <- Pext_npred
		mat_ext_prob[step,] <- ext_prob
		...
		spp_without_preys <- which(rowSums(L) == 0)
		
		pres[step, spp_without_preys] = 0 
	...
	list(pres = pres, 
	     ...
	     mat_ext_prob = mat_ext_prob,
	     mat_Pext_npred = mat_Pext_npred,
	     mat_Pext_sharedpred = mat_Pext_sharedpred)
	})
}
