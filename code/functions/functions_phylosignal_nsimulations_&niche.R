...
compute_cor_phylosig_time_comp.fac <- function(list_sim) {
  ...
  res = data.frame(matrix(ncol=10,nrow=0, dimnames=list(NULL, c("timesteps",
                                                                "phylo cor mean",
                                                                "phylo sign mean",
                                                                "phylo cor pred",
                                                                "phylo sign pred",
  ...
  for (sim in 1:length(list_sim)) {
    ...
    for (i in length(list_interact_distances_pred_corrected)) {
      ...
      colnames(list_niche_dist[[i]])[order(match(colnames(list_dist.phylo_pres[[i]]),colnames(list_dist_niche_dist[[i]])))]
    }
