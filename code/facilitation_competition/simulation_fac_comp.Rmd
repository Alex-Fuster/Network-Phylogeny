---
title: "Simulation communtiy evolution for FACILITATION and COMPETITION"
output: html_notebook
description: this script runs n simulations and save the results for facilitation and competition
---


Note that for facilitation and competition I keep a similar structure than the initial model developed by Mathilde Besson (unpubl.) - the pecularity of this structure, which differs from the simulation code for foodweb and foodweb neutral systems, is that some simulations do not produce large enough communities, so the code selects only those simulations that generate more than n species. Because of this, it includes additional steps to save the seed used in successful simulations (i.e. those producing large enough communities).
...
#### VERSION with beta distrib and fixed range
...
#pars$I_max = 120 # Maximal number of interactioning species
...
#for(b in 1:10){
	...
	for(int in 0:1){
		...
		if(pars$int == 1){
			path <- paste0("../../data/selection_evolution/facilitation_highs/list_res_pos_", successful_sim, ".rds")
	  	saveRDS(res_sim, file = path) #Change if interaction tyep = 0 (competition)
		} else if(pars$int == 0){
			path <- paste0("../../data/selection_evolution/competition_highs/list_res_neg_", successful_sim, ".rds")
	  	saveRDS(res_sim, file = path) #Change if interaction tyep = 0 (competition)
		}
