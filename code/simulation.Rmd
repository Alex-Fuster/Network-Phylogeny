...
#for(b in 1:10){
	...
	for(int in 0:1){
		...
		if(pars$int == 1){
			path <- paste0("../data/selection_evolution/facilitation/list_res_pos_", successful_sim, ".rds")
	  	saveRDS(res_sim, file = path) #Change if interaction tyep = 0 (competition)
		} else if(pars$int == 0){
			path <- paste0("../data/selection_evolution/competition/list_res_neg_", successful_sim, ".rds")
	  	saveRDS(res_sim, file = path) #Change if interaction tyep = 0 (competition)
		}
...
while(successful_sim < nsim+1){
  ...
  # SIMULATION ; from the file code/functions/functions_M-C_bifurcation.R
  simulation <- sim_model_bif(seed = runif(1), pars, nsteps = nsteps)  
  ...
  # Test if we have enough species at the timestep 150 in the simulation
  success <- sum(simulation$pres[40,])
  #print(paste0("Success = ", success))
  ...
  if(success >= 2){
    ...
    res_sim <- list(simulation_number = paste0("simulation", successful_sim), 
                    ...
                    basal = simulation$basal) # Record results from the simulation
   ...
   # list_res <- list_append(list_res, res_sim)
    ...
    successful_sim <- successful_sim + 1 # Count the amount of "good" simulations
      ...
      path <- paste0("../data/selection_evolution/foodweb1/list_res_fw_", successful_sim, ".rds")
...
source("functions/functions_sim_n.R")
...
```{r}
nsteps = 350 # Set the maximum timestep per simulation
nsim = 5 # Set the number of simulations
...
while(successful_sim < nsim+1 & trials != 40){
  ...
  # SIMULATION ; from the file code/functions/functions_M-C_bifurcation.R
  simulation <- sim_model_bif_n(seed = seed, pars, nsteps = nsteps, 
                              pestablishment = df_scenarios[sc, "p_est"],
                              pextinction = df_scenarios[sc, "p_ext"])  
  ...
  if(success >= 10){
    ...
    res_sim <- list(simulation_number = paste0("simulation", successful_sim), 
                    ...
                    list_anc_dist = simulation$list_dist_anc,
                    simulation = nsim,
                    scenario = sc) # Record results from the simulation
    ...
    sc_success[sc] <- "success"
       ...
       path <- paste0("../data/neutral/list_res_n_", successful_sim, "_", sc, ".rds")
	  	saveRDS(res_sim, file = path)  
...
## Run Neutral for chosen scenario


Lets say that after analyzing which scenario is best, I choose scenario 6:
...
while(successful_sim < nsim+1){
  ...
  # SIMULATION ; from the file code/functions/functions_M-C_bifurcation.R
  simulation <- sim_model_bif(seed = seed, pars, nsteps = nsteps, pes = p_est, pex = p_ext )  
  ...
  if(success >= 1){
   ...
   # list_res <- list_append(list_res, res_sim)
    ...
    successful_sim <- successful_sim + 1 # Count the amount of "good" simulations
      ...
      path <- paste0("../data/neutral/sims/list_res_n_", successful_sim, ".rds")
	  	saveRDS(res_sim, file = path)  
  ...
  reccorded_seed_n_list <- list_append(reccorded_seed_n_list, seed_record)
  needed_sim_fw[b] <- total_nb_sim
...
save(reccorded_seed_n_list, file = "../data/neutral/sims/reccorded_seed_n_list1.Rdata")
save(needed_sim_n, file = "../data/neutral/sims/needed_sim_n.Rdata")
