...
# Function to add new elements in the list of groups
...
anc_prim = test$anc
age = nsteps - anc_prim[,1]
S = max(which(age == 1))
anc = anc_prim[1:S,]
rev_anc = apply(anc, 2, rev)

dist_mat = matrix(0,nr = S, nc = S)
...
for(i in 1:(S-1)){
	...
	dist_mat[anc_sp, new_sp] <- nsteps - rev_anc[i,1]
	#dist_mat[new_sp, anc_sp] <- nsteps - rev_anc[i,1]

	group <- lappend(group, c(anc_sp, new_sp))
	anc_duplicats <- grep(anc_sp, group)
	new_duplicats <- grep(new_sp, group)
	if(length(anc_duplicats) > 1){
		for(k in 1:length(anc_duplicats)){
			if(dist_mat[group[[anc_duplicats[k]]][1], new_sp] == 0){
				dist_mat[group[[anc_duplicats[k]]][1], new_sp] <- nsteps - rev_anc[i,1]
				#dist_mat[new_sp, group[[anc_duplicats[k]]][1]] <- nsteps - rev_anc[i,1]
			}
			if(dist_mat[group[[anc_duplicats[k]]][2], new_sp] == 0){
				dist_mat[group[[anc_duplicats[k]]][2], new_sp] <- nsteps - rev_anc[i,1]
				#dist_mat[new_sp, group[[anc_duplicats[k]]][2]] <- nsteps - rev_anc[i,1]
			}
		}
	}
	if(length(new_duplicats) > 1){
		for(g in 1:length(new_duplicats)){
			if(dist_mat[group[[new_duplicats[g]]][1], new_sp] == 0){
				dist_mat[group[[new_duplicats[g]]][1], new_sp] <- nsteps - rev_anc[i,1]
				#dist_mat[new_sp, group[[new_duplicats[g]]][1]] <- nsteps - rev_anc[i,1]
			}
			if(dist_mat[group[[new_duplicats[g]]][2], new_sp] == 0){
				dist_mat[group[[new_duplicats[g]]][2], new_sp] <- nsteps - rev_anc[i,1]
				#dist_mat[new_sp, group[[new_duplicats[g]]][2]] <- nsteps - rev_anc[i,1]
			}
		}
	}
}
...
for(i in 1:(S-1)){
	...
	new_duplicats <- grep(new_sp, group)
	if(length(anc_duplicats) > 1){
		for(k in 1:(length(anc_duplicats)-1)){
			oth_sp <- group[[anc_duplicats[k]]][1]
			if(dist_mat[oth_sp, new_sp] == 0){
				dist_mat[oth_sp, new_sp] <- nsteps - rev_anc[i,1]
			}
			otr_sp <- group[[anc_duplicats[k]]][2]
			if(dist_mat[otr_sp, new_sp] == 0){
				dist_mat[otr_sp, new_sp] <- nsteps - rev_anc[i,1]
			}
		}
	}
	if(length(new_duplicats) > 1){
		for(g in 1:(length(new_duplicats)-1)){
			oth_sp <- group[[new_duplicats[g]]][1]
			if(dist_mat[oth_sp, anc_sp] == 0){
				dist_mat[oth_sp, anc_sp] <- nsteps - rev_anc[i,1]
			}
			otr_sp <- group[[new_duplicats[g]]][2]
			if(dist_mat[otr_sp, anc_sp] == 0){
				dist_mat[otr_sp, anc_sp] <- nsteps - rev_anc[i,1]
			}
		}
	}
}
